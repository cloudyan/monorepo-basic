...
describe('Form', () => {
  ...
  test("`shouldUpdate` shouldn't work with render props", async () => {
    ...
    expect(warnSpy).toHaveBeenCalledWith(
      '[ui-father: Form.Item] `children` of render props only work with `shouldUpdate` or `dependencies`.'
    )
  })
  ...
  test("`shouldUpdate` shouldn't work with `dependencies`", async () => {
    ...
    expect(warnSpy).toHaveBeenCalledWith(
      "[ui-father: Form.Item] `shouldUpdate` and `dependencies` shouldn't be used together."
    )
  })
  ...
  test("`name` shouldn't work with render props", async () => {
    ...
    expect(warnSpy).toHaveBeenCalledWith(
      "[ui-father: Form.Item] Do not use `name` with `children` of render props since it's not a field."
    )
  })
  ...
  test('warning when use `dependencies` but `name` is empty & children is not a render props', async () => {
    ...
    expect(warnSpy).toHaveBeenCalledWith(
      '[ui-father: Form.Item] Must set `name` or use render props when `dependencies` is set.'
    )
  })
  ...
  test('warning with `defaultValue`', async () => {
    ...
    expect(warnSpy).toHaveBeenCalledWith(
      '[ui-father: Form.Item] `defaultValue` will not work on controlled Field. You should use `initialValues` of Form instead.'
    )
  })
  ...
  test('warning when use `name` but children is not validate element', async () => {
    ...
    expect(warnSpy).toHaveBeenCalledWith(
      '[ui-father: Form.Item] `name` is only used for validate React element. If you are using Form.Item as layout display, please remove `name` instead.'
    )
  })
  ...
  describe('helpIcon', () => {
    it('default', () => {
      ...
      expect(baseElement.querySelector('.ui-father-icon')).toBeTruthy()
    })
